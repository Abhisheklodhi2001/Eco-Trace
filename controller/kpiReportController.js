...
const kpiQueue = new Queue("kpi-inventory", {
...
exports.triggerKpiInventoryCalculation = async (req, res) => {
    try {
        ...
        const baseKey = `kpi:${user_id}:${year}`;

        // 🔁 reset redis state
        await redis.del(baseKey);
        await redis.del(`${baseKey}:done`);

        for (const facilityId of facilityList) {
            await kpiQueue.add({ facilityId: facilityId, year, user_id });
        }
        ...
        while (true) {
            const doneCount = await redis.scard(`${baseKey}:done`);

            if (doneCount === facilityList.length) break;

            if (Date.now() - start > MAX_WAIT) {
                return res.status(202).json({
                    success: true,
                    message: "KPI calculation running in background",
                });
            }
        ...
        let results = [];
        for (const facilityId of facilityList) {
            const json = await redis.hget(baseKey, facilityId);
            if (!json) continue;
...
const kpiInventoryTransportVehicle = async (body) => {
    const { facilities, year, user_id } = body;
...
const kpiInventoryBusinessTravel = async (body) => {
    const { facilities, year, user_id } = body;
...
const kpiInventoryEmployeeCommute = async (body) => {
    const { facilities, year, user_id } = body;
...
const kpiInventoryWasteGenerated = async (body) => {
    const { facilities, year, user_id } = body;
...
const kpiInventoryWaterUsage = async (body) => {
    const { facilities, year, user_id } = body;
...
const kpiInventoryGeneralData = async (body) => {
    const { facilities, year, user_id } = body;
...
const kpiInventoryStationaryCombustionde = async (body) => {
...
const kpiInventoryFuelConsumption = async (body) => {
...
const addKpiInventory = async (body) => {
...
kpiQueue.process(async (job) => {
    const { facilityId, year, user_id } = job.data;
    const baseKey = `kpi:${user_id}:${year}`;
    const body = { facilities: facilityId, year, user_id };
    ...
    const [
        ...
        kpiInventory(body),
        kpiInventoryEnergyUse(body),
        kpiInventoryPassengerVehicleEmission(body)
    ]);
    ...
    const [
        ...
        kpiInventoryTransportVehicle(body),
        kpiInventoryBusinessTravel(body),
        kpiInventoryEmployeeCommute(body),
        kpiInventoryWasteGenerated(body)
    ]);
    ...
    const [
        ...
        kpiInventoryWaterUsage(body),
        kpiInventoryGeneralData(body),
        kpiInventoryStationaryCombustionde(body),
        kpiInventoryFuelConsumption({ ...body, type_id: '1' }),
        kpiInventoryFuelConsumption({ ...body, type_id: '2' }),
        kpiInventoryFuelConsumption({ ...body, type_id: '3' })
    ]);
    ...
    await addKpiInventory({ facility_id: facilityId, year, user_id, jsonData: finalJson });

    // 🔐 Redis store
    await redis.hset(baseKey, facilityId, JSON.stringify(finalJson));
    await redis.sadd(`${baseKey}:done`, facilityId);
