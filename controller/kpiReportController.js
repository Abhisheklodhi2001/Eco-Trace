...
exports.triggerKpiInventoryCalculation = async (req, res) => {
    try {
        ...
        const results = [];
        const facilityList = String(facilities).split(",").map(f => f.trim()).filter(Boolean);
        for (const facilityId of facilityList) {
            const body = { facilities: facilityId, year };
            ...
            const [
                ...
                axios.post(`${donfig.live_base_url}kpiInventory`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryEnergyUse`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryPassengerVehicleEmission`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryTransportVehicle`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryBusinessTravel`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryEmployeeCommute`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryWasteGenerated`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryWaterUsage`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryGeneralData`, body, { headers }),
                axios.post(`${donfig.live_base_url}getKpiInventoryStationaryCombustionde`, body, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryFuelConsumption`, { ...body, type_id: '1' }, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryFuelConsumption`, { ...body, type_id: '2' }, { headers }),
                axios.post(`${donfig.live_base_url}kpiInventoryFuelConsumption`, { ...body, type_id: '3' }, { headers })
            ]);
            ...
            const buildFinalInventoryJson = (kpi) => {
                ...
                const fuel1 = normalize12(kpi.fuelConsumption?.type1?.data.length == 12 ? kpi.fuelConsumption?.type1?.data : kpi.fuelConsumption?.type1?.data.slice(1, 13));
                const fuel2 = normalize12(kpi.fuelConsumption?.type2?.data.length == 12 ? kpi.fuelConsumption?.type2?.data : kpi.fuelConsumption?.type2?.data.slice(1, 13));
                const fuel3 = normalize12(kpi.fuelConsumption?.type3?.data.length == 12 ? kpi.fuelConsumption?.type3?.data : kpi.fuelConsumption?.type3?.data.slice(1, 13));
            ...
            await axios.post(`${donfig.live_base_url}addKpiInventory`, { facility_id: facilityId, year, jsonData: finalJson }, { headers });
            results.push({
        ...
        return res.status(200).json({ success: true, message: "KPI Inventory synced successfully", data: results });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ error: true, message: "internal server error " + error.message, success: false });
    }
...
exports.addTriggerKpiTarget = async (req, res) => {
    try {
        const { facility_id, year, jsonData } = req.body;
        const schema = Joi.object({
            facility_id: Joi.string().required(),
...
exports.kpiStep1 = async (req, res) => {
    try {
        const { facilities, year } = req.query;
        ...
        const user_id = req.user.user_id;
        const facilityList = String(facilities)
            .split(",")
            .map(f => f.trim())
            .filter(Boolean);
        ...
        return res.status(200).json({
            success: true,
            message: "Step1 completed successfully",
            data: results
        });
...
exports.kpiStep2 = async (req, res) => {
    const { facilities, year } = req.query;
...
exports.kpiStep3 = async (req, res) => {
    const { facilities, year } = req.query;
...
exports.kpiStep4 = async (req, res) => {
    const { facilities, year } = req.query;
