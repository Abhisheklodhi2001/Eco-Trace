...
const {
  ...
  getScope3CombustionEmission,
  getScope3CompanyOwnedVehicles,
  getScope3Electricity,
  getScope3HeatAndSteam,
    ...
    if (result.error) {
      ...
      if (facilities) {
        ...
        if (array1) {
          ...
          scope1.push(Object.values(monthlyData).map((num) => parseFloat(num / 1000).toFixed(3)));
        }
        ...
        let othermodes_of_transportDetails = await other_modes_of_transportDetails(
          facilitiesdata,
          year,
          finalyeardata
        );
  ...
  try {
    ...
    if (result.error) {
      ...
      if (facilities) {
        ...
        categorydata = await getScope3CombustionEmission(facilitiesdata, year);
        categorydata2 = await getScope3CompanyOwnedVehicles(facilitiesdata, year);
        categorydata3 = await getScope3Electricity(facilitiesdata, year);
        categorydata4 = await getScope3HeatAndSteam(facilitiesdata, year);
      }
