...
exports.Addrenewableelectricity = async (req, res) => {
  try {
    ...
    if (result.error) {
      const message = result.error.details.map((i) => i.message).join(",");
      return res.json({
        message: result.error.details[0].message, error: message,
        missingParams: result.error.details[0].message, status: 200, success: false
      });
    } else {
      let user_id = req.user.user_id;
      let countrydata = await country_check(facilities);
      if (countrydata.length == 0) return res.json({ success: false, message: "EF not found", status: 400 });
      ...
      if (SubCategorySeedID == '9') {
        ...
        if (year >= startYear && year <= endYear) {
          category = {
            ...
            Active: 1,
            SendForApproval: 'yes',
            scop3_GHGEmission: parseFloat(refunit[0].kgCO2e_kwh) * readingValue,
            FileName: req.file != undefined ? req.file.filename : null
          }
        } else {
          return res.json({
        ...
        if (typeID == '1') {
          ...
          if (!ef_factor || ef_factor.length == 0) return res.json({ success: false, message: "EF not found for renewable energy", status: 400 });
          ...
          category = {
            ...
            scop3_GHGEmission: '0.00',
            scope3_emission_factor: '0.00',
            FileName: req.file != undefined ? req.file.filename : null
          }
        } else {
          let where = `where  item = 'T&D - Electricity' and  is_scope3 = '1' and country_id = ${countrydata[0].CountryId} and Right(Fiscal_Year, 4) = '${year}'`;
          const ef_factor = await getData('`dbo.electricity`', where);
          if (!ef_factor || ef_factor.length == 0) return res.json({ success: false, message: "EF not found for renewable energy", status: 400 });
          ...
          category = {
            ...
            Unit: unit ? unit : "",
            note: note ? note : "",
            GHGEmission: parseFloat(readingValue * emission_factor),
            SubCategorySeedID: SubCategorySeedID ? SubCategorySeedID : "",
            typeID: typeID ? typeID : "",
            readingValue: readingValue ? readingValue : "",
            year: year ? year : "",
            ...
            scop3_GHGEmission: parseFloat(readingValue * ef_factor[0].kgCO2e_kwh),
            scope3_emission_factor: ef_factor[0].kgCO2e_kwh,
            FileName: req.file != undefined ? req.file.filename : null
          }
        }
      }
      ...
      if (allinsertedID.length > 0) {
        return res.json({ success: true, message: "Succesfully Added renewable electricity", categories: '', status: 200 });
      } else {
        return res.json({ success: false, message: "Some problem occured while Adding renewable electricity", status: 500 });
      }
    }
  } catch (err) {
    console.log(err);
...
exports.Addheatandsteam = async (req, res) => {
  try {
    ...
    if (result.error) {
      ...
      let refunit;
      if (unit == 'Kwh') {        
        refunit = await getheatandsteam(typeID, SubCategorySeedID, countrydata[0].CountryId, year);
      } else {
        refunit = await getheatandsteam(typeID, SubCategorySeedID, countrydata[0].CountryId, year);
        refunit[0].kgCO2e_kwh = refunit[0].kgCO2e_rth;
        refunit[0].scop3_kgCO2e_kwh = refunit[0].scop3_kgCO2e_rth;
      }
