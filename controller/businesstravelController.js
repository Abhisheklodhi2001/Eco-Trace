...
const {
  ...
  insert_water_discharge_by_destination, insert_water_discharge_by_destination_only, insertprocessing_of_sold_productsCategory, insertprocessing_of_sold_products_ef, findFlightSpendEmissionFactor } = require("../models/businesstravel");
...
exports.OtherModesOfTransportNodeTypeByTypeName = async (req, res) => {
  const { type_name, facility_id, year } = req.body;
  try {
    const schema = Joi.object({
    ...
    let countrydata = await country_check(facility_id);
    const findTypeNameResponse = await getSelectedColumn("other_modes_of_transport_ef", 'WHERE type_name = "' + type_name + '" AND country_id = "' + countrydata[0].CountryId + '" and Right(Fiscal_Year,4) = "' + year + '"', "id, mode_type, type_name, country_id");
    if (findTypeNameResponse.length > 0) {
      return res.status(200).json({ error: false, message: "Data fetched successfully", success: true, data: findTypeNameResponse });
    } else {
      return res.status(404).json({ error: true, message: "No records found", success: false });
...
exports.flightTravel = async (req, res) => {
  const { flight_calc_mode, reading_value, jsonData, year, month, facilities } = req.body;
  try {
    const schema = Joi.object({
      flight_calc_mode: Joi.string().required(),
      reading_value: [Joi.number().optional()],
    ...
    if (flight_calc_mode == 'Spend Based') {
      ...
      const EFVRes = await findFlightSpendEmissionFactor(countrydata[0].CountryId, year);
      if (EFVRes.length == 0) {
      ...
      const flighttravel = {
        ...
        reference_id: 0,
        distance: 0,
        emission: parseFloat(reading_value * EFV).toFixed(2),
        spend_base_value: reading_value,
        emission_factor: EFV,
      ...
      for (const val of parsedData) {
        try {
          ...
          if (countrydata.length > 0) {
            ...
            let where = ` where mode_type = '${type}' and type_name = '${mode_of_trasport}' and country_id = '${countrydata[0].CountryId} AND Right(Fiscal_Year,4) = '${year}'`;
...
exports.UploadWaterSupplyDE = async (req, res) => {
  try {
    ...
    if (result.error) {
      ...
      let insertMonth = []
      let countrydata = await country_check(facilities);
      if (countrydata.length == 0) return res.json({ success: false, message: "EF not Found for water supply treatment category", status: 400 });
      ...
      if (water_supply) {
        ...
        if (watersupplyEf.length > 0) {
          ...
          return res.json({ success: false, message: "EF not Found for this year", status: 400 });
        }
      } else {
        return res.json({ success: false, message: "EF not Found for water supply category", status: 400 });
      }
      ...
      let category = {
        ...
        emission_factor_unit: water_supply_unit == 1 ? "kg C02e/KL" : "kg C02e/m3",
        facilities: facilities ? facilities : "",
        user_id: user_id,
        batch: batch,
        year: year
...
//           json.map(async (item, index) => {
//             for (let i = 0; i < insertId.length; i++) {
//               let waterSupply = JSON.parse(water_supply);
//               let waterSupplyNext = 0;
//               if(waterSupply?.length  > 0)
//               {
//                  waterSupplyNext = waterSupply[0];
//               }
//               else
//               {
//                  waterSupplyNext = waterSupply;
//               }

//               let totalwaterwithdrawl = parseFloat(Number(waterSupplyNext) * item.kilolitres / 100);
//               let withdrawn_emission = totalwaterwithdrawl * waterSupplyEfValues[index];
...
//       console.log("âœ… Final totalWithdrawnEmission:", totalWithdrawnEmission);
...
//           json.map(async (item) => {
//             for (let i = 0; i < insertId.length; i++) {
//               let waterTreatment = JSON.parse(water_treatment);
//               let waterTreatmentNext = 0;
//               if(waterTreatment?.length  > 0)
//               {
//                  waterTreatmentNext = Number(waterTreatment[0]);
//               }
//               else
//               {
//                  waterTreatmentNext = Number(waterTreatment);
//               }
//               let totaldischarge = parseFloat(waterTreatmentNext * item.kilolitres / 100);
//               dischargearray.push(totaldischarge);
//               // let category = {
//               //   water_discharge: item.type ? item.type : "",
//               //   kilolitres: item.kilolitres ? item.kilolitres : 0,
...
//       let waterTreatmentOne = JSON.parse(water_treatment);
//       let waterTreatmentOneNext = 0;
//       if(waterTreatmentOne?.length  > 0)
//       {
//            waterTreatmentOneNext = Number(waterTreatmentOne[0]);
//       }
//       else
//       {
//             waterTreatmentOneNext = Number(waterTreatmentOne);
//       }
//       let non_water_treated = waterTreatmentOneNext - totalWaterTreated;
//       let totalEmissions = totalTreatedEmission + totalWithdrawnEmission;
//       if (insertId) {
//         for (let i = 0; i < insertId.length; i++) {
//           const waterpplyory1 = await updateWater_ef(totalEmissions, totalWithdrawnEmission, totalTreatedEmission, parseFloat(totalWaterTreated).toFixed(4), non_water_treated, totalWaterTreated, insertId[i]);
...
exports.vehicleCategories_lease = async (req, res) => {
  try {
    let where = " where  lease_status = '1' order BY id ASC";
...
exports.vehicleSubCategories_lease = async (req, res) => {
  try {
    const { id, facility_id, year } = req.query;
    let countrydata = await country_check(facility_id);
    let where = `where vehicle_category_id = ${id} AND country_id = ${countrydata[0].CountryId} and Right(Fiscal_Year,4) = ${year} GROUP BY vehicle_type`;
