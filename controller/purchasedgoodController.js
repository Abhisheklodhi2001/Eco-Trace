...
const {
  ...
  fetchVendorCountry, fetchPurchaseGoodDataBulk, fetchEmissionByCountry, fetchSpendBaseEmissionByCountry
...
exports.purchaseGoodsAllcategoriesFilter = async (req, res) => {
  try {
    const { typeofpurchase, country_id, year } = req.body;

    const schema = Joi.object({
      typeofpurchase: Joi.string().required(),
    ...
    const purchase_goods = await fetchPurchase_categories(
      product_code_id,
      typeofpurchase
    );
    ...
    if (purchase_goods.length > 0) {
      await Promise.all(
        purchase_goods.map(async (item) => {
          const sub_categories = await fetchPurchase_subcategories(
            product_code_id,
            typeofpurchase,
            item.value
          );
          ...
          await Promise.all(
            sub_categories.map(async (item1) => {
              const categoriesproduct =
                await fetchPurchase_categoriesproduct(
                  product_code_id,
                  typeofpurchase,
                  item1.value,
                  item.value,
...
exports.downStreamTransportation = async (req, res) => {
  try {
    ...
    const schema = Joi.alternatives(
      Joi.object({
        vehicle_type: [Joi.string().optional().empty()],
        spent_base: [Joi.string().optional().empty()],
        currency: [Joi.string().optional().empty()],
        reading_value: [Joi.string().optional().empty()],
        sub_category: [Joi.string().optional().empty()],
        noOfVehicles: [Joi.number().optional().empty()],
        mass_of_products: [Joi.number().optional().empty()],
        distanceInKms: [Joi.number().optional().empty()],
    ...
    if (checkNUllUnDString(spent_base)) {
      const EFVRes = await fetchSpendBaseEmissionByCountry(countrydata[0].CountryId, year);
      if (EFVRes.length == 0) {
        return res.json({ success: false, message: "No country found for this EF", status: 400 });
      } else {
        let yearRange = EFVRes[0]?.Fiscal_Year;
      ...
      if (checkNUllUnD(EFV)) {
        let totalEmission = parseFloat(reading_value * EFV).toFixed(2);
        downStreamData.vehicleType = null;
        downStreamData.subCategory = null;
        downStreamData.noOfVehicles = null;
        downStreamData.massOfProducts = mass;
        downStreamData.distanceInKms = distancekm;
        downStreamData.emissionVehicle = totalEmission;
        downStreamData.spend_base_value = reading_value;
        downStreamData.emission_factor_value = EFV;
        downStreamData.emission_factor_value_unit = `kg co2e/${currency}`;
        downStreamData.userId = user_id;
        downStreamData.status = "P";
        downStreamData.storageFType = "";
        downStreamData.areaOccupied = 0;
    ...
    if (checkNUllUnDString(storagef_type)) {
      ...
      downStreamData.averageNoOfDays = averageNoOfDays;
      downStreamData.emissionStorage = totalEmission;
      downStreamData.spend_base_value = reading_value ? reading_value : null;
      downStreamData.unit_of_mass = mass_unit ? mass_unit : "";
      downStreamData.emission_factor_storage = EFS;
      downStreamData.emission_factor_storage_unit = "co2e tonnes/m2.day";
      downStreamData.userId = user_id;
  ...
  try {
    ...
    if (checkNUllUnDString(vehicle_type)) {
      ...
      if (checkNUllUnD(EFV)) {
        ...
        downStreamData.distanceInKms = distancekm;
        downStreamData.emissionVehicle = totalEmission;
        downStreamData.spend_base_value = null;
        downStreamData.emission_factor_value = EFV;
        downStreamData.emission_factor_value_unit = "kg co2e/km.tonnes";
        downStreamData.userId = user_id;
        downStreamData.status = "P";
...
exports.vehicleSubCategories = async (req, res) => {
  try {
    ...
    let countrydata = await country_check(facility_id);
    let where = `where vehicle_category_id = ${id} AND country_id = ${countrydata[0].CountryId} and Right(Fiscal_Year,4) = '${year}'`;
