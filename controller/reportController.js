...
const {
  ...
  getVehicle, getPurchaseGoodsReportCount, getReportCombustionEmissionFuel, getReportAllelectricityFuel, getReportAllheatandsteamScope3, getReportAllVehiclede, getSingleReportCount
...
const {
  ...
  getElecricityReportMultiNewScope3,
  getHeatandSteamReportMultiNew,
  getHeatandSteamReportMultiNewScope3,
  getWaterSupplyNew,
  getWaterTreatmentNew,
  getStationaryCombustionReportMultiNew,
  getStationaryCombustionReportMultiNewScope3,
  getWaterDischarge,
  getWaterDischargeByYear,
  getWaterWithdrawal,
  getWaterWithdrawalByYear,
  getVendorsReportMulti,
  getVendorsReportConsole,
  getConpanyOwnedVehiclesMultiNew,
  getConpanyOwnedVehiclesMultiNewScope3,
  getVendorsCount,
...
exports.reportFilterPurchaseGoods = async (req, res) => {
  try {
    const { facility, year, month, page, page_size, category, subcategory } = req.body;
...
exports.reportFuelAndEnergyRelated = async (req, res) => {
  try {
    const { facility, year, month, page, page_size, category, subcategory } = req.body;
    const schema = Joi.alternatives(
      Joi.object({
    ...
    let categorydata = await getReportCombustionEmissionFuel(user_id, facility, year, month);
    let categorydata1 = await getReportAllelectricityFuel(user_id, facility, year, month);
    let categorydata2 = await getReportAllheatandsteamScope3(user_id, facility, year, month);
    let categorydata3 = await getReportAllVehiclede(user_id, facility, year, month);
    let array = [...categorydata, ...categorydata1, ...categorydata2, ...categorydata3];

    if (category) {
      array = array.filter(item => item.tablename === category);
    }

    if (subcategory) {
      if (category === "Heat and Steam") { 
        array = array.filter(item => item.typeName === subcategory); 
      } else {
        array = array.filter(item => item.subcatName === subcategory);
      }
    }
    ...
    await Promise.all(
      paginatedData.map(async (item) => {
        ...
        if (item.status == "S") item.status = "Approved";
        if (item.status == "R") item.status = "Rejected";
        if (item.status == "P") item.status = "Pending";
        let userid = "";
        item.user_id == "uuidd12432" ? (userid = 4) : (userid = item.user_id);
        // if (item.user_id == "uuidd12432") {
        //   userid = 4;
        // } else {
        //   userid = item.user_id;
        // }
        const tenants = await getSelectedColumn("`dbo.tenants`", `where Id  = '${userid}'`, "*");
        tenants.length > 0 ? (item.user_name = tenants[0].tenantName) : (item.user_name = "");
        // if (tenants.length > 0) {
        //   item.user_name = tenants[0].tenantName;
        // } else {
        //   item.user_name = "";
        // }
        if (item.tablename == "Stationary Combustion") emissionFactor = item.Scope3GHGEmissionFactor;
        if (item.tablename == "Electricity") {
          if (item.SubCategorySeedID == "1002") {
            const Renewable = await getSelectedColumn("`dbo.electricity`", ` where SubCategorySeedID ='${item.SubCategorySeedID}'`, "*");
            if (item.typeID == "1") {
            ...
            if (((unit.toLowerCase() === "kwh") != item.typeID) == "2") {
              emissionFactor = Renewable[0]["kgCO2e_kwh"] ? Renewable[0]["kgCO2e_kwh"] : "0.000";
            } else {
              emissionFactor = item.emission_factor;
            }
          } else {
            const Renewable = await getSelectedColumn("`dbo.electricity`", ` where is_scope3 ='1'`, "*");
        ...
        if (item.tablename == "Heat and Steam") {
          const heatandsteam = await getSelectedColumn("`dbo.heatandsteam`", `where id = '${item.typeID}'`, "*");
          item.subcatName = heatandsteam[0]["Item"] ? heatandsteam[0]["Item"] : "";
          item.unit.toLowerCase() == "kwh" ? (emissionFactor = heatandsteam[0]["scop3_kgCO2e_kwh"]) : (emissionFactor = heatandsteam[0]["scop3_kgCO2e_rth"]);

          // if (item.unit.toLowerCase() == "kwh") {
          //   emissionFactor = heatandsteam[0]["scop3_kgCO2e_kwh"];
          // } else {
          //   emissionFactor = heatandsteam[0]["scop3_kgCO2e_rth"];
          // }
        }

        if (item.tablename == "Company Owned Vehicles") {
          item.TypeName = item.typeName;
          emissionFactor = item.Scope3_GHGEmission_factor;
        }

        item.emission = parseFloat(item.emission);
        item.emissionFactorUsed = parseFloat(emissionFactor).toFixed(3);
      })
    );
    if (paginatedData.length > 0) {
      for (elem of paginatedData) {
        var tempObj = {
          category_name: elem.tablename,
          subcategory: elem.subcatName,
          quantity: elem.quantity,
          unit: elem.unit,
          emission: elem.emission,
          facility: elem.facilities,
          year: elem.year,
          month: elem.month,
          user_name: elem.user_name,
        };
...
exports.reportProOfSoldProducts = async (req, res) => {
  try {
    ...
    const [totalCount] = await getSingleReportCount(
      ...
      year,
      monthsCondition,
      "processing_of_sold_products_category",
      "facilities"
    )
  ...
  try {
    ...
    if (reportResult.length > 0) {
      reportResult.map((elem) => {
        elem.item = elem.SubCategorySeedID == '9' ? "Location Based" : "Market Based";
      });
      return res.json({
        success: true,
        status: 200,
  ...
  try {
    ...
    const schema = Joi.alternatives(
      Joi.object({
        ...
        fuel_and_energy_related: [Joi.number().required()]
      })
    );
    const result = schema.validate(req.body);
    if (result.error) {
    ...
    var fuelAndEnergyRelated = Number(fuel_and_energy_related);
    ...
    if (fuelAndEnergyRelated) {
      const processResults = async (year, months) => {
        const reportResult = await getStationaryCombustionReportMultiNewScope3(facility, year, months);
        const reportResult1 = await getElecricityReportMultiNewScope3(facility, year, months);
        const reportResult2 = await getHeatandSteamReportMultiNewScope3(facility, year, months);
        const reportResult3 = await getConpanyOwnedVehiclesMultiNewScope3(facility, year, months);

        if (reportResult.length > 0) multiReport.push(...reportResult);

        if (reportResult1.length > 0) {
          reportResult1.forEach(element => {
            if (element.DataPoint1 == 1) {
              element.DataPoint1 = "Renewable Energy Cert (REC)";
            } else if (element.DataPoint1 == 2) {
              element.DataPoint1 = "Supplier Specific";
          ...
          multiReport.push(...reportResult1);  // FIXED
        }

        if (reportResult2.length > 0) multiReport.push(...reportResult2);

        if (reportResult3.length > 0) {
          for (let elem of reportResult3) {
            multiReport.push(elem);
          }
        }
      };
      ...
      // ---------------------------
      if (differenceInYear === 0) {
        const months = getMonths(start_month, end_month);
        await processResults(start_year, months);
      }
      ...
      // ---------------------------
        ...
        for (let year = Number(start_year); year <= Number(end_year); year++) {
          ...
          await processResults(year, months);
        }
      }
    }
  ...
  try {
    ...
    if (waterSupplyTreatment) {
      if (differenceInYear == 0) {
        ...
        if (reportResult.length > 0 && reportResult[0].Emission != null) {
          multiReport.push(...reportResult);
        }
        ...
        if (reportResult1.length > 0 && reportResult1[0].Emission != null) {
          multiReport.push(...reportResult1);
        }
      } else {
        for (var year = Number(start_year); year <= Number(end_year); year++) {
...
exports.waterReport = async (req, res) => {
  try {
    const { facility, start_year, start_month, end_year, end_month, data_type } = req.body;
    ...
    const schema = Joi.alternatives(
      Joi.object({
        ...
        end_year: [Joi.string().empty().required()],
        end_month: [Joi.string().empty().required()],
        data_type: [Joi.string().required().allow('monthly', 'consolidated')],
      })
    );
    ...
    if (data_type && data_type === 'consolidated') {
      var differenceInYear = Number(end_year) - Number(start_year);
      var waterWithdrawal = [];
      var waterDischarge = [];
      ...
      if (differenceInYear == 0) {
        ...
        waterWithdrawal = await getWaterWithdrawalByYear(facility, year);
        waterDischarge = await getWaterDischargeByYear(facility, year);
        waterDischargeOnly = await getWaterDischargeOnlyByYear(facility, year);
        ...
        finalResult = finalResult.filter(
          m => m.waterWithdrawal.length > 0 ||
            m.waterDischarge.length > 0 ||
            m.waterDischargeOnly.length > 0
        );
      } else {
        for (var year = Number(start_year); year <= Number(end_year); year++) {
          waterWithdrawal = await getWaterWithdrawalByYear(facility, year);
          ...
          if (
            yearlyData.waterWithdrawal.length > 0 ||
            yearlyData.waterDischarge.length > 0 ||
            yearlyData.waterDischargeOnly.length > 0
          ) {
            finalResult.push(yearlyData);
          }
        }
      }
      ...
      if (differenceInYear == 0) {
        ...
        let waterWithdrawal1 = await getWaterWithdrawal(facility, year, months);
        waterWithdrawal.push(...waterWithdrawal1);
        let waterDischarge1 = await getWaterDischarge(facility, year, months);
        waterDischarge.push(...waterDischarge1);
        let waterDischargeOnly1 = await getWaterDischargeOnly(facility, year, months);
        waterDischargeOnly.push(...waterDischargeOnly1);

        let monthsNew = "Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec";
        let monthsArr = monthsNew.split(",").map(m => m.trim());
        let yearlyResult = monthsArr.map(month => {
          return {
            month,
            year,
            waterWithdrawal: waterWithdrawal.filter(
              w => w.month === month && String(w.year) == String(year)
            ),
            waterDischarge: waterDischarge.filter(
              w => w.month === month && String(w.year) == String(year)
            ),
            waterDischargeOnly: waterDischargeOnly.filter(
              w => w.month === month && String(w.year) == String(year)
            )
          };
        });

        yearlyResult = yearlyResult.filter(
          m => m.waterWithdrawal.length > 0 ||
            m.waterDischarge.length > 0 ||
            m.waterDischargeOnly.length > 0
        );

        finalResult.push(...yearlyResult);
      } else {
        for (var year = Number(start_year); year <= Number(end_year); year++) {
          var months = "";
          if (year === Number(start_year)) {
