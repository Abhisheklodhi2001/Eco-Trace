    ...
    try {
        ...
        if (result.error) {
            ...
            if (facilities) {
                ...
                if (categorydata4.length > 0) {
                    for (item of categorydata4) {
                        if (item.month_number &&
                            monthlyData2.hasOwnProperty(item.month_number)) {
                            monthlyData2[item.month_number] += parseFloat(
                                item?.scope3_emission ? item?.scope3_emission : 0
                ...
                const combinedCategories = ['Stationary Combustion', 'Electricity', 'Heat and Steam', 'Company Owned Vehicles'];
                ...
                if (combinedEmission > 0) {
                    resultArray2.unshift({
                        emission: parseFloat(combinedEmission.toFixed(3)),
...
exports.fetchScope1EmissionData = async (req, res) => {
    try {
        ...
        const [RefrigerantEmissionData = null] = await getRefrigerantEmissionData(facilityIds, year);
        const [ExtinguisherEmissionData = null] = await getExtinguisherEmissionData(facilityIds, year);
        const [DieselPassengerData = null] = await getDieselPassengerData(facilityIds, year);
        const [PetrolPassengerData = null] = await getPetrolPassengerData(facilityIds, year);
        const [DieselDeliveryData = null] = await getDieselDeliveryData(facilityIds, year);
        const [otherDeliveryData = null] = await getOtherDeliveryData(facilityIds, year);
    ...
    try {
        ...
        if (result.error) {
            ...
            if (facilities) {
                ...
                categorydata3 = await upstream_vehicle_storage_emissions(facilitiesdata, year);
                categorydata4 = await downstream_vehicle_storage_emissions(facilitiesdata, year);
                categorydata5 = await waste_generated_emissionsDetails(facilitiesdata, year);
                ...
                categorydata12 = await getAllcompanyownedvehicles(
                    facilitiesdata,
                    year
                );
                array3 = [
                ...
                if (categorydata11.length > 0) {
                    for (item of categorydata11) {
                        if (item.month_number &&
                            monthlyData2.hasOwnProperty(item.month_number)) {
                            monthlyData2[item.month_number] += parseFloat(
                                item?.scope3_emission ? item?.scope3_emission : 0
                ...
                if (categorydata12.length > 0) {
                    for (item of categorydata12) {
                        if (item.month_number &&
                            monthlyData2.hasOwnProperty(item.month_number)) {
                            monthlyData2[item.month_number] += parseFloat(
                                item?.scope3_emission ? item?.scope3_emission : 0
                ...
                Object.keys(categoryTotals3).forEach((category) => {
                    const emission = categoryTotals3[category] / 1000;
...
exports.ghgEnergyConsumptionWellTank = async (req, res) => {
    try {
        ...
        if (result.error) {
            ...
            categorydata = await getScope3CombustionEmission(facilities, year);
            categorydata2 = await getScope3CompanyOwnedVehicles(facilities, year);
            categorydata3 = await getScope3Electricity(facilities, year);
            categorydata4 = await getScope3HeatAndSteam(facilities, year);
            ...
            let sumtotal1 = parseFloat(Number(totalsum) + Number(totalsum1) + Number(totalsum2) + Number(totalsum3));
            ...
            return res.json({
                ...
                series: ["Stationary combustion", "Company owned vehicles", "Electricity", "Heat and Steam"],
                status: 200,
            });
        }
    } catch (error) {
...
exports.ghgEnergyConsumptionMonth = async (req, res) => {
    try {
        ...
        if (result.error) {
            ...
            if (facilities) {
                categorydata = await getCombustionEmission(facilities, year, finalyeardata);
                if (categorydata) {
                    await Promise.all(
                ...
                categorydata4 = await getAllcompanyownedvehicles(facilities, year, finalyeardata);
                categorydata5 = await getAllelectricity(facilities, year, finalyeardata);
                categorydata6 = await getAllheatandsteam(facilities, year, finalyeardata);
                if (categorydata5) {
                    await Promise.all(
                ...
                let categorydata3 = [...categorydata, ...categorydata4, ...categorydata5, ...categorydata6];
    ...
    try {
        ...
        if (result.error) {
            ...
            if (facilities) {
                ...
                const category4Scope3 = categorydata4.map(item => ({
                    YEAR: Number(item.Year || item.YEAR || item.year),
                    emission: Number(item.scope3_emission) || 0
                }));

                const category5Scope3 = categorydata5.map(item => ({
                    YEAR: Number(item.Year || item.YEAR || item.year),
                    emission: Number(item.scope3_emission) || 0
                }));

                const category6Scope3 = categorydata6.map(item => ({
                    YEAR: Number(item.Year || item.YEAR || item.year),
                    emission: Number(item.scope3_emission) || 0
                }));

                const combinedArray3 = [...normalizedArray3, ...categoryScope3, ...category4Scope3, ...category5Scope3, ...category6Scope3];
    ...
    try {
        ...
        if (result.error) {
            ...
            if (facilities) {
                ...
                const category1Scope3 = categorydata4.map(item => ({
                    YEAR: Number(item.Year || item.YEAR || item.year),
                    emission: Number(item.scope3_emission) || 0
                }));

                const category2Scope3 = categorydata5.map(item => ({
                    YEAR: Number(item.Year || item.YEAR || item.year),
                    emission: Number(item.scope3_emission) || 0
                }));

                const category3Scope3 = categorydata6.map(item => ({
                    YEAR: Number(item.Year || item.YEAR || item.year),
                    emission: Number(item.scope3_emission) || 0
                }));

                const combinedArray3 = [...normalizedArray3, ...categoryScope3, ...category1Scope3, ...category2Scope3, ...category3Scope3];
