  ...
  try {
    ...
    if (result.error) {
      ...
      if (id == '7') {
        ...
        stationarycombustion = await getpassengervehicletypes(user_id, year);
      } else if (id == '11') {
        stationarycombustion = await getdeliveryvehicletypesWithCountryId(countrydata[0].CountryId, year);
      } else if (id == '9') {
        stationarycombustion = await getelectricity(id, countrydata[0].CountryId, year);
      } else if (id == '1002') {
        stationarycombustion = await getRenewableelectricity(id, countrydata[0].CountryId, year);
      } else if (id == '12') {
        stationarycombustion = await getheatandsteam(id, countrydata[0].CountryId, year);
      } else {
        let where = ` where SubCategorySeedID ='${id}' AND country_id = '${countrydata[0].CountryId}' and Right(Fiscal_Year, 4) = ${year} ORDER BY item ASC`;
  ...
  try {
    ...
    await Promise.all(
      facilities.map(async (item) => {
        let where = ` JOIN \`dbo.categoryseeddata\` C ON MD.ManageDataPointCategorySeedID = C.Id WHERE MD.ManageDataPointId = '${item.ID}' GROUP BY manageDataPointCategorySeedID ORDER BY manageDataPointCategorySeedID ASC`;
        ...
        await Promise.all(
          managePointCategories.map(async (item1) => {
            let where1 = ` LEFT JOIN subcategoryseeddata C ON MD.ManageDataPointSubCategorySeedID = C.Id WHERE MD.ManageDataPointCategoriesId = '${item1.ID}' GROUP BY MD.ManageDataPointSubCategorySeedID, C.Item ORDER BY MD.ManageDataPointSubCategorySeedID ASC`;
...
exports.addMultipleCompanyOwnedVehicles = async (req, res) => {
  try {
    ...
    if (result.error) {
      ...
      if (!countrydata.length) return res.status(400).json({ error: true, message: "EF not found while adding company owned vehicles", success: false });
      ...
      for (let val of vehicles) {
          ...
          if (year >= startYear && year <= endYear) {
            if (charging_outside != "undefined") {
              if (val.unit == 'Km') {
                ...
                scop3_efs = parseFloat(refunit[0].scope3_kgCO2e_km);
              }
              if (val.unit == 'Litre') {
                efs = parseFloat(refunit[0].kgCO2e_litre);
                scop3_efs = parseFloat(refunit[0].scope3_kgCO2e_litre);
              }
              let emissionef = efs + parseFloat(charging_outside) / 100 * parseFloat(refunit[0].kgCO2e_kg);
              let scope3_emissionef = scop3_efs + parseFloat(charging_outside) / 100 * parseFloat(refunit[0].scop3_kgCO2e_kg);
              ...
              scop3_GHGEmission = val.is_excel == 1 ? parseFloat(val.value) * parseFloat(scope3_emissionef) : no_of_vehicles * trip_per_vehicle * parseFloat(val.value) * parseFloat(scope3_emissionef);
            } else {
              if (val.unit == 'Km') {
                efs = parseFloat(refunit[0].kgCO2e_km);
                scop3_efs = parseFloat(refunit[0].scope3_kgCO2e_km);
                ...
                scop3_GHGEmission = val.is_excel == 1 ? (parseFloat(refunit[0].scope3_kgCO2e_km) * parseFloat(val.value)) : (parseFloat(refunit[0].scope3_kgCO2e_km) * no_of_vehicles * trip_per_vehicle * parseFloat(val.value));
              }
              if (val.unit == 'Litre') {
                efs = parseFloat(refunit[0].kgCO2e_litre);
                scop3_efs = parseFloat(refunit[0].scope3_kgCO2e_litre);
                ...
                scop3_GHGEmission = val.is_excel == 1 ? (parseFloat(refunit[0].scope3_kgCO2e_litre) * parseFloat(val.value)) : (parseFloat(refunit[0].scope3_kgCO2e_litre) * no_of_vehicles * trip_per_vehicle * parseFloat(val.value));
              }
              if (val.unit != 'Km' && val.unit != 'Litre') {
                efs = parseFloat(refunit[0].kgCO2e_ccy);
                scop3_efs = parseFloat(refunit[0].scope3_kgCO2e_ccy);
                ...
                scop3_GHGEmission = val.is_excel == 1 ? (parseFloat(refunit[0].scope3_kgCO2e_ccy) * parseFloat(val.value)) : (parseFloat(refunit[0].scope3_kgCO2e_ccy) * no_of_vehicles * trip_per_vehicle * parseFloat(val.value));
              }
            }
          } else if (year == startYear) {
            if (charging_outside != "undefined") {
        ...
        let category = {
          ...
          note: note ? note : "",
          GHGEmission: GHGEmission,
          GHGEmission_factor: efs,
          Scope3_GHGEmission: scop3_GHGEmission,
          Scope3_GHGEmission_factor: scop3_efs,
