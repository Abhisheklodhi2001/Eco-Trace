...
const {
  ...
  getAllelectricityFuel,
  getAllheatandsteamScope3,
  ...
  purchase_goods_categories_ef_by_match_productCategory_Id, deleteById, findCountryNamebyCode, deleteEntries, purchase_goods_matched_items_ai_by_payload_id_and_status_paginated, findAssetsFileLogo, findRedirectUrl
} = require("../models/user");
        ...
        array.map(async (item) => {
          ...
          if (categoryID == "13") {
            ...
            if (item.tablename == "flight_travel") {
              if (item.spend_base_value == null) {
                item.unit = "km";
              } else {
                let countrydata = await country_check(facilities);
                const where = ` where ID = '${countrydata[0].CountryId}'`;
                const countryCode = await getSelectedColumn(
                  "`dbo.country`",
                  where,
                  "*"
                );

                item.unit = countryCode[0].CurrencyCode;
              }
              emissionFactor = item.emission_factor;
            }
          ...
          if (categoryID == "8") {
            ...
            item.product_code_name = item.productcodestandard == 1 ? "HSN" : item.productcodestandard == 2 ? "NAIC" : item.productcodestandard == 3 ? "ISIC" : "";
  ...
  try {
    ...
    if (result.error) {
      ...
      if (categoryID == "9") {
        ...
        let categorydata1 = await getAllelectricityFuel(facilities, year);
        // console.log("categorydata1 =>", categorydata1);
        let categorydata2 = await getAllheatandsteamScope3(facilities, year);
        let categorydata3 = await getAllVehiclede(facilities, year);
        // console.log("categorydata3 =>", categorydata3);
        array = [...categorydata, ...categorydata1, ...categorydata2, ...categorydata3];
      }
      await Promise.all(
        array.map(async (item) => {
          const category = await getSelectedColumn(
          ...
          if (item.tablename == "dbo.heatandsteamde") {
            ...
            item.TypeName = heatandsteam[0]["Item"]
              ? heatandsteam[0]["Item"]
              : "";
            item.subcatName = heatandsteam[0]["Item"]
              ? heatandsteam[0]["Item"]
              : "";
...
exports.downloadExcelTemlate = async (req, res) => {
  try {
    ...
    const excelSheetName = await findAssetsFileLogo();
    switch (Number(categoryID)) {
      case 1:
        fileName = `${excelSheetName[0].fireextinguisher_template}`;
        downloadName = "FireExtinguisher_Template.xlsx";
        break;
      case 2:
        fileName = `${excelSheetName[0].refrigerant_template}`;
        downloadName = "Refrigerant_Template.xlsx";
        break;
      case 3:
        fileName = `${excelSheetName[0].vehiclefleet_deliveryvehicle}`;
        downloadName = "VehicleFleet-deliveryVehicle.xlsx";
        break;
      case 4:
      default:
        fileName = `${excelSheetName[0].vehiclefleet_passengervehicle}`;
        downloadName = "VehicleFleet-passengervehicle.xlsx";
        break;
    }
    const workbook = new ExcelJS.Workbook();
    const filePath = path.resolve(__dirname, "../public/excel_template/", fileName);
    ...
    // Example: update all sheets dynamically
    workbook.worksheets.forEach(sheet => {
      sheet.getCell("A1").value = `Updated: ${sheet.name}`;
    });

    // ✅ Send directly to response without overwriting template
...
exports.getExcelSheet = async (req, res) => {
  try {
    ...
    const fetchLoginLogo = await findAssetsFileLogo();

    if (!fetchLoginLogo) return res.status(404).json({ error: true, message: "purchasedGoods template not found", success: false });
    const workbook = new ExcelJS.Workbook();
    const filePath = path.resolve(__dirname, `../public/excel_template/${fetchLoginLogo[0].purchased_goods}`);
...
exports.downloadExcelVehicleFleetByFacilityCategoryId = async (req, res) => {
  try {
    ...
    const fetchLoginLogo = await findAssetsFileLogo();
    if (!fetchLoginLogo) return res.status(404).json({ error: true, message: "vehiclede template not found", success: false });

    const workbook = new ExcelJS.Workbook();
    const filePath = path.resolve(__dirname, `../public/excel_template/${fetchLoginLogo[0].vehiclede}`);
...
exports.loginLogo = async (req, res) => {
  try {
    const fetchLoginLogo = await findAssetsFileLogo();
    if (fetchLoginLogo.length > 0) {
      fetchLoginLogo[0].fav_icon = `${config.live_base_url}${fetchLoginLogo[0].fav_icon}`;
      fetchLoginLogo[0].login_logo = `${config.live_base_url}${fetchLoginLogo[0].login_logo}`;
      fetchLoginLogo[0].dashboard_logo = `${config.live_base_url}${fetchLoginLogo[0].dashboard_logo}`;
      fetchLoginLogo[0].fireextinguisher_template = `${config.live_base_url}excel_template/${fetchLoginLogo[0].fireextinguisher_template}`;
      fetchLoginLogo[0].refrigerant_template = `${config.live_base_url}excel_template/${fetchLoginLogo[0].refrigerant_template}`;
      fetchLoginLogo[0].vehiclefleet_deliveryvehicle = `${config.live_base_url}excel_template/${fetchLoginLogo[0].vehiclefleet_deliveryvehicle}`;
      fetchLoginLogo[0].vehiclefleet_passengervehicle = `${config.live_base_url}excel_template/${fetchLoginLogo[0].vehiclefleet_passengervehicle}`;

      return res.status(200).json({ error: false, success: true, message: "Login image found", data: fetchLoginLogo });
    } else {
      return res.status(404).json({ error: true, success: false, message: "Login image not found", data: null });
    }
  } catch (error) {
    console.log(error);

    return res.status(500).json({ error: true, success: false, message: "Internal server error " + error.message });
  }
};

exports.dasboardLogo = async (req, res) => {
  try {
    const fetchDashboardLogo = await findAssetsFileLogo();
    if (fetchDashboardLogo.length > 0) {
      fetchDashboardLogo[0].filename = `${config.base_url}${fetchDashboardLogo[0].dashboard_logo}`;
      delete fetchDashboardLogo[0].login_logo;
      return res.status(200).json({ error: false, success: true, message: "Login image found", data: fetchDashboardLogo });
    } else {
      return res.status(404).json({ error: true, success: false, message: "Login image not found", data: null });
    }
  } catch (error) {
...
exports.redirectUrl = async (req, res) => {
  try {
    const fetchRedirectUrl = await findRedirectUrl();
    if (fetchRedirectUrl.length > 0) {
      return res.status(200).json({ error: false, success: true, message: "Redirect URL found", data: fetchRedirectUrl });
    } else {
      return res.status(404).json({ error: true, success: false, message: "Redirect URL not found", data: null });
    }
  } catch (error) {
    return res.status(500).json({ error: true, success: false, message: "Internal server error " + error.message });
  }
