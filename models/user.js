...
module.exports = {
    ...
    getAllelectricityFuel: async (facilities, year) => {
        return db.query(`
            SELECT 
              'dbo.renewableelectricityde' AS tablename,
              S.Id AS subcategoryID,
    ...
    getAllheatandsteamScope3: async (facilities, year) => {
        return db.query("SELECT COALESCE('dbo.heatandsteamde', '') AS tablename, S.Id AS subcategoryID, S.Item AS subcatName, A.Unit AS unit, CASE WHEN A.Status = 'P' THEN 'Pending' ELSE 'Approved' END AS status, A.months AS month, A.scop3GHGEmission AS emission, B.Item AS typeName, A.* FROM `dbo.heatandsteamde` A LEFT JOIN subcategoryseeddata S ON S.Id = A.SubCategorySeedID LEFT JOIN `dbo.heatandsteam` B ON B.ID = A.typeID WHERE A.facilities = ? AND A.year = ? ORDER BY A.id DESC;", [facilities, year]);
    },

    getAllVehiclede: async (facilities, year) => {
    ...
    findAssetsFileLogo: async () => {
        return await db.query('SELECT * FROM `assets_file`')
    },

    findRedirectUrl: async () => {
        return await db.query('SELECT redirect_url FROM `assets_file` order by id desc limit 1')
    }
}