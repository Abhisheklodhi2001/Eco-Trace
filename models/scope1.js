...
module.exports = {
  ...
  getdeliveryvehicletypesWithCountryId: async (country_id) => {
    return db.query('select * from `dbo.deliveryvehicletypes` where vehicletypes_id < 4  AND country_id = "' + country_id + '" and Right(Fiscal_Year, 4) = "' + year + '"');
  },
  ...
  getelectricity: async (id, CountryId, year) => {
    return db.query('select *,SubCategorySeedID as subCatTypeID from `dbo.electricity` where SubCategorySeedID = "' + id + '" AND country_id = "' + CountryId + '" and Right(Fiscal_Year, 4) = "' + year + '"');
  },


  getRenewableelectricity: async (id, CountryId, year) => {
    return db.query('select *,SubCategorySeedID as subCatTypeID from `dbo.electricity` where SubCategorySeedID = "' + id + '" AND country_id = "' + CountryId + '" and Right(Fiscal_Year, 4) = "' + year + '"');
  },


  getheatandsteam: async (id, CountryId, year) => {
    return db.query('select * from `dbo.heatandsteam` where SubCategorySeedID = "' + id + '" AND country_id = "' + CountryId + '" and Right(Fiscal_Year, 4) = "' + year + '"');
  },
