...
module.exports = {
  ...
  fetchEmissionByCountry: async (country_id, year) => {
    return db.query("select * from vehicle_subcategory where country_id = ? and Right(Fiscal_Year, 4) = ? ", [country_id, year])
  },

  fetchSpendBaseEmissionByCountry: async (country_id, year) => {
    return await db.query("select * from transport_spend_base_emissionfactor where country_id = ? and Right(Fiscal_Year, 4) = ? ", [country_id, year]);
  },
