...
module.exports = {
  ...
  getheatandsteam: async (typeID, SubCategorySeedID, country_id, year) => {
    return db.query("select * from `dbo.heatandsteam` where subCatTypeID = ? AND SubCategorySeedID =? and country_id=? and Right(Fiscal_Year, 4) = ?", [typeID, SubCategorySeedID, country_id, year]);
  },
  //Addheatandsteam
