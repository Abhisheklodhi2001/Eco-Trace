...
module.exports = {
    ...
    getKpiInventoryByFacilitityAndYearAndUserId: async (facility_id, year, user_id) => {
        return await db.query('SELECT * FROM `tbl_kpi_inventory` WHERE facility_id = "' + facility_id + '" AND year = "' + year + '" AND user_id = "' + user_id + '"');
    },

    deleteKpiInventoryByFacilitityAndYearAndUserId: async (facility_id, year, user_id) => {
        return await db.query('DELETE FROM `tbl_kpi_inventory` WHERE facility_id = ? AND year = ? AND user_id = ?', [facility_id, year, user_id]);
    },
    ...
    getKpiInventoryJsonByFacilitityAndYearAndUserId: async (facility_id, year, user_id) => {
        return await db.query('SELECT tbl_kpi_inventory_json.*, B.firstname, B.lastname FROM `tbl_kpi_inventory_json` LEFT JOIN `dbo.aspnetusers` B ON B.user_id = tbl_kpi_inventory_json.user_id WHERE tbl_kpi_inventory_json.facility_id = "' + facility_id + '" AND tbl_kpi_inventory_json.year = "' + year + '" AND tbl_kpi_inventory_json.user_id = "' + user_id + '"');
    },

    deleteKpiInventoryJsonByFacilitityAndYearAndUserId: async (facility_id, year, user_id) => {
        return await db.query('DELETE FROM `tbl_kpi_inventory_json` WHERE facility_id = ? AND year = ? AND user_id = ?', [facility_id, year, user_id]);
    },
