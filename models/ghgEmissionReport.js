...
module.exports = {
    ...
    getAllcompanyownedvehicles: async (facilities, year, finalyeardata) => {
        ...
        return db.query("select  SUM(A.GHGEmission) as emission,SUM(A.Scope3_GHGEmission) AS scope3_emission,A.months AS month_number,COALESCE('Company Owned Vehicles', '')  as  category    from `dbo.vehiclede` A  " + where);
    },
...
LIMIT 1; ', [facility_id, year]);
    ...
    getCombustionEmissionData: async (facility_id, year) => {
        return db.query(`SELECT ROUND(SUM(A.GHGEmission) / 1000, 4) AS Total_GHGEmission, A.TypeName 
                    FROM stationarycombustionde A 
                    WHERE A.facility_id IN (${facility_id}) AND A.Year IN(${year}) AND A.Status = 'S'
                    GROUP BY A.TypeName 
    ...
    getRefrigerantEmissionData: async (facility_id, year) => {
        try {
            return await db.query(`SELECT ROUND(SUM(r.GHGEmission) / 1000, 4) AS Total_GHGEmission, ref.Item AS TypeName FROM \`dbo.refrigerantde\` r LEFT JOIN ( SELECT DISTINCT subCatTypeID, Item FROM \`dbo.refrigents\` ) ref ON ref.subCatTypeID = r.subCategoryTypeId WHERE r.facilities IN (${facility_id}) AND r.Status = "S" AND r.year IN (${year}) ORDER BY Total_GHGEmission DESC;`)
        } catch (error) {
    ...
    getExtinguisherEmissionData: async (facility_id, year) => {
        try {
            return await db.query(`SELECT ROUND(SUM(GHGEmission) / 1000, 4) AS Total_GHGEmission, facilities, year FROM \`dbo.fireextinguisherde\` WHERE facilities IN (${facility_id}) AND year IN (${year}) AND STATUS = "S" GROUP BY facilities, year;`)
        } catch (error) {
            console.error("Database Error:", error);
            throw error;
        }
    ...
    getDieselPassengerData: async (facility_id, year) => {
        return await db.query(`SELECT ROUND(SUM(v.GHGEmission) / 1000, 4) AS Total_GHGEmission, v.facilities, companyownedvehicles.ItemType FROM \`dbo.vehiclede\` AS v LEFT JOIN companyownedvehicles ON v.VehicleTypeId = companyownedvehicles.ID WHERE v.facilities IN (${facility_id}) AND v.year IN (${year}) AND companyownedvehicles.ItemType LIKE "%Diesel%" AND v.Status = "S" and v.SubCategorySeedID = 10 GROUP BY v.facilities, companyownedvehicles.ItemType;`);
    },

    getPetrolPassengerData: async (facility_id, year) => {
        return await db.query(`SELECT ROUND(SUM(v.GHGEmission) / 1000, 4) AS Total_GHGEmission, v.facilities, companyownedvehicles.ItemType FROM \`dbo.vehiclede\` AS v LEFT JOIN companyownedvehicles ON v.VehicleTypeId = companyownedvehicles.ID WHERE v.facilities IN (${facility_id}) AND v.year = ${year} AND companyownedvehicles.ItemType LIKE "%Petrol%" AND v.Status = "S" and v.SubCategorySeedID = 10 GROUP BY v.facilities, companyownedvehicles.ItemType;`);
    },

    getDieselDeliveryData: async (facility_id, year) => {
        try {
            return await db.query(`SELECT ROUND(SUM(v.GHGEmission) / 1000, 4) AS Total_GHGEmission, v.facilities, companyownedvehicles.ItemType FROM \`dbo.vehiclede\` AS v LEFT JOIN companyownedvehicles ON v.VehicleTypeId = companyownedvehicles.ID WHERE v.facilities IN (${facility_id}) AND v.year IN (${year}) AND companyownedvehicles.ItemType LIKE "%Diesel%" AND v.Status = "S" and v.SubCategorySeedID = 11 GROUP BY v.facilities, companyownedvehicles.ItemType;`);
        } catch (error) {
            console.error("Database Error:", error);
            throw error;
        }
    ...
    getOtherDeliveryData: async (facility_id, year) => {
        return await db.query(`SELECT \`dbo.vehiclede\`.id, ROUND(SUM(\`dbo.vehiclede\`.GHGEmission) / 1000, 4) AS Total_GHGEmission, \`dbo.vehiclede\`.vehicleTypeID, companyownedvehicles.ItemType FROM \`dbo.vehiclede\` LEFT JOIN companyownedvehicles ON companyownedvehicles.ID = \`dbo.vehiclede\`.vehicleTypeID WHERE \`dbo.vehiclede\`.facilities IN(${facility_id}) AND \`dbo.vehiclede\`.year IN (${year}) AND \`dbo.vehiclede\`.SubCategorySeedID = 11 AND \`dbo.vehiclede\`.Status = "S" AND companyownedvehicles.ItemType NOT LIKE "%Diesel%" GROUP BY \`dbo.vehiclede\`.facilities, companyownedvehicles.ItemType;`);
    },
...
AND pgc.year = ? 
AND pgc.typeofpurchase = ? AND pgc.status = 'S'
...
LIMIT 5;
    ...
    flightTravel: async (facilities, year) => {
        return await db.query(`SELECT flight_travel.*, ROUND(SUM(flight_travel.emission) / 1000, 4) AS total_emission FROM flight_travel WHERE facilities IN (${facilities}) AND flight_calc_mode != 'Spend Based' AND year = ? AND status = "S" GROUP BY flight_Type;`, [year]);
    },
    ...
    getScope3CombustionEmission: async (facilities, year) => {
        let where = "";
        where = ` where  A.Year = '${year}' and Status = 'S' `;
        if (facilities != '0') {
            where += `  and A.facility_id IN (${facilities})`
    ...
    getScope3CompanyOwnedVehicles: async (facilities, year) => {
        let where = "";
        where = ` where  A.year = '${year}' and STATUS = 'S' `;
        if (facilities != '0') {
            where += `  and  A.facilities IN (${facilities})`
        }
        return db.query(`select SUM(A.Scope3_GHGEmission) as emission from \`dbo.vehiclede\` A ${where}`);
    },
    getScope3Electricity: async (facilities, year) => {
        let where = "";
        where = ` where  A.year = '${year}' and STATUS = 'S' `;
        if (facilities != '0') {
            where += `  and  A.facilities IN (${facilities})`
        }
        return db.query(`select SUM(A.scop3_GHGEmission) as emission from \`dbo.renewableelectricityde\` A ${where}`);
    },
    getScope3HeatAndSteam: async (facilities, year) => {
        let where = "";
        where = ` where  A.year = '${year}' and STATUS = 'S' `;
        if (facilities != '0') {
            where += `  and  A.facilities IN (${facilities})`
        }
        return db.query(`select SUM(A.scop3GHGEmission) as emission from \`dbo.heatandsteamde\` A ${where}`);
    },
    ...
    getAllcompanyownedvehiclesRangeWise: async (facilities, base_year, current_year, finalyeardata) => {
        ...
        return db.query("select A.Year, ROUND(SUM(A.GHGEmission) / 1000, 4) as emission, ROUND(SUM(A.Scope3_GHGEmission) / 1000, 4) AS scope3_emission from `dbo.vehiclede` A  " + where);
    },
    ...
    getAllelectricityRangeWise: async (facilities, base_year, current_year, finalyeardata) => {
        ...
        return db.query("select A.YEAR, ROUND(SUM(A.GHGEmission) / 1000, 4) AS emission, ROUND(SUM(A.Scop3_GHGEmission) / 1000, 4) AS scope3_emission from `dbo.renewableelectricityde` A " + where);
    },
    ...
    getAllheatandsteamRangeWise: async (facilities, base_year, current_year, finalyeardata) => {
        ...
        return db.query("select A.Year, ROUND(SUM(A.GHGEmission) / 1000, 4) as emission, ROUND(SUM(A.Scop3GHGEmission) / 1000, 4) AS scope3_emission from `dbo.heatandsteamde` A " + where);
    },
