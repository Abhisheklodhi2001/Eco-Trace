...
module.exports = {
  getPurchaseGoodsReportMultiNew: async (facility, year, month) => {
    ...
    'Scope3' AS scope,
    'Purchase Goods & Services' AS category,  -- from productCategory table
    sc.name AS DataPoint1, -- from subCategory table
...
WHERE 
    ...
    AND pgc.status = 'S'
GROUP BY 
    category, DataPoint1, DataPoint2, Months, Years, facility
ORDER BY 
...
`);
  ...
  getConpanyOwnedVehiclesMultiNew: async (facility, year, month) => {
    return db.query(`select 'Scope1' as scope, 'Company Owned Vehicles' as category,cov.Item as DataPoint1,cov.ItemType  as DataPoint2, \
                     ...
                     vd.facilities= f.ID  group by DataPoint1, DataPoint2, Months, Years, facility     \
                     ORDER BY FIELD(vd.MONTHS,'Jan','Feb','Mar','Apr', 'May','Jun',     \
                    'Jul','Aug','Sep','Oct','Nov', 'Dec')`);
  },

  getConpanyOwnedVehiclesMultiNewScope3: async (facility, year, month) => {
    return db.query(`select 'Scope3' as scope, 'Company Owned Vehicles' as category,cov.Item as DataPoint1,cov.ItemType  as DataPoint2, \
                      f.AssestName as facility,   sum(vd.Scope3_GHGEmission) as Emission,  vd.year as Years, vd.months as Months   \
                      from  \`dbo.vehiclede\` vd, \`dbo.facilities\` f,  \`companyownedvehicles\`   cov   \
                      where   vd.facilities in (${facility}) and  vd.status = 'S' and \
                      vd.year =${year} and vd.months in (${month}) and vd.vehicleTypeID = cov.id and    \
                      vd.facilities= f.ID  group by DataPoint1, DataPoint2, Months, Years, facility     \
  ...
  getDownStreamVehicleReportMultiNew: async (facility, year, month) => {
    return db.query(`select 'Scope3' as scope, 'Downstream Vehicles' as category, vehicletypes.vehicle_type AS DataPoint1, sub_category as DataPoint2,  \
                    ...
                    group by DataPoint1, DataPoint2, Months, Years, facility \ 
  ...
  getInvestmentEmissionReportMultiNew: async (facility, year, month) => {
    return db.query(`  select 'Scope3' as scope, 'Investment Emissions' as category, ie.category as Industry_Sector,  ie.sub_category as Sub_Sector, ie.investement_type as Type_of_Investment, ie.calculation_method as Data_Type,  \
                      ...
                      group by Industry_Sector, Sub_Sector, Type_of_Investment, Data_Type, Months, Years, sub_group  \
                      ORDER BY FIELD(MONTH,'Jan','Feb','Mar','Apr', 'May','Jun',     \             
                      'Jul','Aug','Sep','Oct','Nov', 'Dec')`);
  },
  ...
  getStationaryCombustionReportMultiNewScope3: async (facility, year, month) => {
    return db.query(` select 'Scope3' as scope, 'Stationary Combustions' as category, scsd.Item as DataPoint1, TypeName as DataPoint2,  \
                      sum(sc.Scope3GHGEmission) as Emission,  sc.year as Years, sc.month as Months,  \
                      f.AssestName as facility from  stationarycombustionde sc, \`dbo.facilities\`  f,  \
                      subcategoryseeddata scsd   \
                      where   sc.facility_id in (${facility}) and  \
                      sc.year =${year}  and sc.month in (${month}) and   sc.status = 'S' and \
  ...
  getEmployeeCommutingReportMultiNew: async (facility, year) => {
    ...
    return db.query(
      `select 'Scope3' as scope, 'Employee Commuting' as category, ectt.type as DataPoint1, ecc.totalnoofcommutes as DataPoint2,  \
                     ...
                     group by DataPoint1, DataPoint2 ,Months, Years, facility ORDER BY FIELD(MONTH,'Jan','Feb','Mar','Apr', 'May','Jun', 'Jul','Aug','Sep','Oct','Nov', 'Dec')`
    );
  },
  ...
  getHomeOfficeReportMultiNew: async (facility, start_year, end_year) => {
    return db.query(`select 'Scope3' as scope, 'Home Office' AS category, homeoffice_emission_factors.typeof_homeoffice AS DataPoint1, "" AS DataPoint2, year, month, sum(emission) as Emission, facilities as facility  \
                    from  homeoffice_category, homeoffice_emission_factors  where status  ='S'  and facilities in (${facility}) and year >='${start_year}' and year <= '${end_year}' AND homeoffice_emission_factors.id = homeoffice_category.typeofhomeoffice GROUP BY DataPoint1, year, month, facility \
                    ORDER BY FIELD(MONTH,'Jan','Feb','Mar','Apr', 'May','Jun',                  
                    'Jul','Aug','Sep','Oct','Nov', 'Dec')`);
  },
  ...
  getElecricityReportMultiNew: async (facility, year, month) => {
    return db.query(`select 'Scope' as scope,  scsd.Item as category, rende.typeID as DataPoint1, rende.SourceName as DataPoint2,  \
                     ...
                     f.AssestName as facility from  \`dbo.renewableelectricityde\`  rende, \`dbo.facilities\`  f, subcategoryseeddata scsd \
                      ...
                      group by category, DataPoint1, Months, Years, facility ORDER BY FIELD(MONTHS,'Jan','Feb','Mar','Apr', 'May','Jun',                  
                    'Jul','Aug','Sep','Oct','Nov', 'Dec')`);
  },

  getElecricityReportMultiNewScope3: async (facility, year, month) => {
    return db.query(`select 'Scope3' as scope,  'Electricity' as category, rende.typeID as DataPoint1, rende.SourceName as DataPoint2,  \
                     sum(rende.scop3_GHGEmission) as Emission,  rende.year as Years, rende.months as Months,  \
                     f.AssestName as facility from  \`dbo.renewableelectricityde\`  rende, \`dbo.facilities\`  f, subcategoryseeddata scsd \
                      where   rende.facilities in (${facility})   \
                      and rende.year =${year}  and rende.months in (${month})  and rende.status ='S' \
                      and  rende.facilities= f.ID  \
  ...
  getHeatandSteamReportMultiNew: async (facility, year, month) => {
    return db.query(`select 'Scope1' as scope,  scsd.Item as category, hns.Item as DataPoint1,    \
                     ...
                     group by DataPoint1, Months, Years, facility ORDER BY FIELD(MONTHS,'Jan','Feb','Mar','Apr', 'May','Jun',                  
                    'Jul','Aug','Sep','Oct','Nov', 'Dec')`);
  },

  getHeatandSteamReportMultiNewScope3: async (facility, year, month) => {
    return db.query(`select 'Scope3' as scope,  scsd.Item as category, hns.Item as DataPoint1,    \
                     sum(hnsde.scop3GHGEmission) as Emission,  hnsde.year as Years, hnsde.months as Months,    \
                     f.AssestName as facility from  \`dbo.heatandsteamde\`  hnsde, \`dbo.facilities\`  f,   \
                     \`dbo.heatandsteam\` hns, subcategoryseeddata scsd   \
                     where   hnsde.facilities in (${facility})   \
                     and hnsde.year =${year} and hnsde.months in (${month}) and hnsde.status ='S'  \
  ...
  getWaterSupplyNew: async (facility, year, month) => {
    return db.query(`select 'Scope3' as scope, 'Water Supply and Treatment' as category, 'Water Supply' as DataPoint1, '-' as DataPoint2, SUM(hnsde.withdrawn_emission) AS Emission, hnsde.year as Years, hnsde.month as Months,    \
                     ...
                     and  hnsde.facilities= f.ID   \
                     group by Months, Years, facility ORDER BY FIELD(MONTHS,'Jan','Feb','Mar','Apr', 'May','Jun',                  
                    'Jul','Aug','Sep','Oct','Nov', 'Dec')`);
  },
  ...
  getWaterDischarge: async (facility, year, month) => {
    return db.query(`SELECT
    wdd.leveloftreatment AS Category,
      SUM(wdd.withthtreatment) AS Total,
    SUM(
        CASE WHEN wdd.water_discharge = 'Into Surface water' THEN wdd.withthtreatment ELSE 0
    END
) AS "Surface water",
SUM(
    CASE WHEN wdd.water_discharge = 'Into Ground water' THEN wdd.withthtreatment ELSE 0
END
) AS "Ground water",
SUM(
    CASE WHEN wdd.water_discharge = 'Send to third-parties' THEN wdd.withthtreatment ELSE 0
END
) AS "Third party",
SUM(
    CASE WHEN wdd.water_discharge = 'Into Seawater' THEN wdd.withthtreatment ELSE 0
END
) AS "Sea water",
SUM(
    CASE WHEN wdd.water_discharge = 'Others' THEN wdd.withthtreatment ELSE 0
END
) AS "Others",
wstg.month,
wstg.year,
wstg.emission_factor_unit as unit
FROM
    water_discharge_by_destination wdd
JOIN water_supply_treatment_category wstg ON
    wstg.id = wdd.water_supply_treatment_id
WHERE
    wstg.facilities IN(${facility}) AND wstg.month in (${month}) and wstg.year=${year} AND wstg.status = 'S' AND wdd.leveloftreatment != ''
GROUP BY
    wdd.leveloftreatment,
    wstg.month,
    wstg.year
ORDER BY
    Category ASC,
  ...
  getWaterDischargeByYear: async (facility, year) => {
    return db.query(`SELECT
    wdd.leveloftreatment AS Category,
      SUM(wdd.withthtreatment) AS Total,
    SUM(
...
JOIN water_supply_treatment_category wstg ON
    wstg.id = wdd.water_supply_treatment_id
WHERE
    wstg.facilities IN(${facility}) and wstg.year=${year} AND wstg.status = 'S' AND wdd.leveloftreatment != ''
GROUP BY
    wdd.leveloftreatment,
    wstg.year
ORDER BY
  ...
  getWaterDischargeOnly: async (facility, year, month) => {
    return db.query(`SELECT "Water Discharged" AS Category,
    SUM(wbs.totaldischarge) AS Total,
    SUM(
        CASE WHEN wbs.water_discharge = 'Surface water' THEN wbs.totaldischarge ELSE 0
    END
) AS "Surface water",
SUM(
    CASE WHEN wbs.water_discharge = 'Groundwater' THEN wbs.totaldischarge ELSE 0
END
) AS "Ground water",
SUM(
    CASE WHEN wbs.water_discharge = 'Third party water' THEN wbs.totaldischarge ELSE 0
END
) AS "Third party",
SUM(
    CASE WHEN wbs.water_discharge = 'Sea water / desalinated water' THEN wbs.totaldischarge ELSE 0
END
) AS "Sea water",
SUM(
    CASE WHEN wbs.water_discharge = 'Others' THEN wbs.totaldischarge ELSE 0
END
) AS "Others",
wstg.month,
wstg.year,
wstg.emission_factor_unit as unit
FROM
    water_discharge_by_destination_only wbs
JOIN water_supply_treatment_category wstg ON
    wstg.id = wbs.water_supply_treatment_id
WHERE
    wstg.facilities IN(${facility}) AND wstg.month IN(${month}) AND wstg.year = ${year} AND wstg.status = 'S'
GROUP BY
    wstg.month,
    wstg.year
ORDER BY
  ...
  getWaterDischargeOnlyByYear: async (facility, year) => {
    return db.query(`SELECT "Water Discharged" AS Category,
    SUM(wbs.totaldischarge) AS Total,
    SUM(
        CASE WHEN wbs.water_discharge = 'Surface water' THEN wbs.totaldischarge ELSE 0
...
JOIN water_supply_treatment_category wstg ON
    wstg.id = wbs.water_supply_treatment_id
WHERE
    wstg.facilities IN(${facility}) AND wstg.year = ${year} AND wstg.status = 'S'
GROUP BY
    wstg.year
ORDER BY
    wstg.year,
  ...
  getWaterWithdrawal: async (facility, year, month) => {
    return db.query(`SELECT "Water Withdrawn" AS Category,
    SUM(wbs.totalwaterwithdrawl) AS Total,
    SUM(
        CASE WHEN wbs.water_withdrawl = 'Surface water' THEN wbs.totalwaterwithdrawl ELSE 0
    END
) AS "Surface water",
SUM(
    CASE WHEN wbs.water_withdrawl = 'Groundwater' THEN wbs.totalwaterwithdrawl ELSE 0
END
) AS "Ground water",
SUM(
    CASE WHEN wbs.water_withdrawl = 'Third party water' THEN wbs.totalwaterwithdrawl ELSE 0
END
) AS "Third party",
SUM(
    CASE WHEN wbs.water_withdrawl = 'Sea water / desalinated water' THEN wbs.totalwaterwithdrawl ELSE 0
END
) AS "Sea water",
SUM(
    CASE WHEN wbs.water_withdrawl = 'Others' THEN wbs.totalwaterwithdrawl ELSE 0
END
) AS "Others",
wstg.month,
wstg.year,
wstg.emission_factor_unit as unit
FROM
    water_withdrawl_by_source wbs
JOIN water_supply_treatment_category wstg ON
    wstg.id = wbs.water_supply_treatment_id
WHERE
    wstg.facilities IN(${facility}) AND wstg.month IN(
        ${month}
    ) AND wstg.year = ${year} AND wstg.status = 'S'
GROUP BY
    wstg.month,
    wstg.year
ORDER BY
  ...
  getWaterWithdrawalByYear: async (facility, year) => {
    return db.query(`SELECT "Water Withdrawn" AS Category,
    SUM(wbs.totalwaterwithdrawl) AS Total,
    SUM(
        CASE WHEN wbs.water_withdrawl = 'Surface water' THEN wbs.totalwaterwithdrawl ELSE 0
...
WHERE 
  ...
  getProductGraphVendors: async (facility, year) => {
    return db.query(`select  pgcef.product as Product,
                     ...
                     where  pgc.status ='S' and pgc.facilities in (${facility}) and pgc.year = ${year}
